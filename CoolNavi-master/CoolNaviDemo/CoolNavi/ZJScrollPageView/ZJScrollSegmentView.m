...
static CGFloat const xGap = 0;//5.0;
static CGFloat const wGap = 0;//2*xGap;
static CGFloat const contentSizeXOff = 0.0;//20.0;
...
- (void)setupTitles
{
    ...
    self.segmentStyle.titleFont = [UIFont systemFontOfSize:18.f];
    NSInteger index = 0;
...
{
    self.segmentStyle.scrollLineHeight = 4.0;
    CGFloat titleX = 0.0;
    CGFloat titleY = 0.0;
    CGFloat titleW = 0.0;
    ...
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        ...
        NSInteger index = 0;
        self.segmentStyle.titleMargin = 0.0;
    ...
    if (self.scrollLine)
    {
        if (self.segmentStyle.isScrollTitle) {
            ...
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                coverW = [self.titleWidths[_currentIndex] floatValue];
                coverX = (firstLabel.zj_width - coverW) * 0.5;
            }
            self.scrollLine.frame = CGRectMake(coverX , self.frame.size.width * 1.0 / self.titleViews.count, coverW , self.segmentStyle.scrollLineHeight);
        }
    }
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex
{
    ...
    {
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.zj_x = oldTitleView.zj_x + xDistance * progress + (oldTitleView.zj_width / 2.0 - oldTitleWidth / 2.0);
            self.scrollLine.zj_width = oldTitleWidth + wDistance * progress;
        }
        else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
...
- (UIView *)scrollLine
{
    ...
    if (!_scrollLine) {
        ...
        self.segmentStyle.scrollLineColor = [UIColor redColor];
