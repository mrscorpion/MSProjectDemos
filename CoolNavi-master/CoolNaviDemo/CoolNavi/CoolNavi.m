...
//        _backImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, -0.5*frame.size.height, frame.size.width, frame.size.height*1.5)];
        ...
        CGFloat headerImgW = 100;
        _headerImageView = [[UIImageView alloc] initWithFrame:CGRectMake(frame.size.width*0.5-headerImgW*0.5, 0.27*frame.size.height, headerImgW, headerImgW)];
        ...
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0.5*frame.size.height, frame.size.width, frame.size.height*0.2)];
        _titleLabel.textAlignment = NSTextAlignmentCenter;
        _titleLabel.font = [UIFont systemFontOfSize:20];
        _titleLabel.text = title;
        
        _subTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0.6*frame.size.height, frame.size.width, frame.size.height*0.1)];
...
//    CGFloat imageReduce = 1-(newOffset.y-startChangeOffset)/(d*2);
    CGFloat imageReduce = 1-(newOffset.y-startChangeOffset)/(d*1.6);
    self.subTitleLabel.alpha = alpha;
    CGFloat titleAlpha = (1 - (newOffset.y-startChangeOffset)/d * 4.0 > 0.0) ? (1-(newOffset.y-startChangeOffset)/d * 4.0) : 0.0;
...
//    self.backImageView.frame = CGRectMake(0, -0.5*self.frame.size.height+(1.5*self.frame.size.height-64)*(1-alpha), self.backImageView.frame.size.width, self.backImageView.frame.size.height);
    self.backImageView.frame = CGRectMake(0, -0.5*self.frame.size.height+(1.5*self.frame.size.height-64)*(1-alpha), self.frame.size.width, self.frame.size.height*1.5);
...
//    self.titleLabel.frame = CGRectMake(0, 0.5*self.frame.size.height+(subviewOffset-0.45*self.frame.size.height)*(1-alpha), self.frame.size.width, self.frame.size.height*0.2);
    self.titleLabel.frame = CGRectMake(0, 12 + (subviewOffset-0.6*self.frame.size.height)*(1-alpha), self.frame.size.width, self.frame.size.height*0.2);
    self.subTitleLabel.frame = CGRectMake(0, 0.65*self.frame.size.height+(subviewOffset-0.45*self.frame.size.height)*(1-alpha), self.frame.size.width, self.frame.size.height*0.1);
}
